      ...
      <div className="card" style={{ padding: '0', display: 'flex', flexDirection: 'column', overflow: 'hidden', marginBottom: '2rem' }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button className="btn btn-secondary" onClick={() => setQuestion('')}>Clear</button>
            <button 
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                    {result.record.columns.map(c => (
                      <th key={c} style={{ padding: '0.75rem 1rem', color: 'var(--text-muted)', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{c}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {result.record.rows.map((row, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      {result.record.columns.map(c => (
                        <td key={c} style={{ padding: '0.875rem 1rem', fontSize: '0.875rem' }}>{row[c]?.toString() || '-'}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
